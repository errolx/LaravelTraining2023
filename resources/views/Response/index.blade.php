                ...
                @forelse ($sheets as $sheets)
                    @if ($sheets->dept)
                        @if (date('Y-m-d', strtotime($sheets->Date)) >= date('Y-m-d', strtotime($datestart)) &&
                                date('Y-m-d', strtotime($sheets->Date)) <= date('Y-m-d', strtotime($dateend)) && $sheets->comments)
                            <tr>
    ...
    <div class="table-settings mb-4">
        <div class="row align-items-center justify-content-between">
            <div class="col col-md-6 col-lg-3 col-xl-4">
                <form action="{{ route('responses.index') }}" method="GET">
                    <div class="input-group me-2 me-lg-3 fmxw-400">
						<input type="text" name="search" value="{{ $searchVal }}" class="form-control" placeholder="Department" required>
                        <input type="date" name="datestart" value="{{ $datestart }}" class="form-control"
                            placeholder="" required>
                        <input type="date" name="dateend" value="{{ $dateend }}" class="form-control"
                            placeholder="" required>
                ...
                <tbody>
                    <tr>
                        <td valign="middle"><span class="fw-normal">
                                ...
                                @foreach ($sheets as $sheets01)
                                    @if ($sheets01->dept)
                                        @if ($sheets01->dept == $searchVal && date('Y-m-d', strtotime($sheets01->Date)) >= date('Y-m-d', strtotime($datestart)) &&
                                                date('Y-m-d', strtotime($sheets01->Date)) <= date('Y-m-d', strtotime($dateend)))
                                            @if ($sheets01->score >= 9)
                                                @php
                                                    $ctrpro = $ctrpro + 1;
        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                @forelse ($sheets as $sheets02)
                    @if ($sheets02->dept)
                        @if ($sheets02->dept == $searchVal && date('Y-m-d', strtotime($sheets02->Date)) >= date('Y-m-d', strtotime($datestart)) &&
                                date('Y-m-d', strtotime($sheets02->Date)) <= date('Y-m-d', strtotime($dateend)) && $sheets02->comments)
                            <tr>
                                <td valign="middle"><span
                                        class="fw-normal">{{ date('Y-m-d', strtotime($sheets02->Date)) }}</span></td>
                    ...
                    {{--
                                ...
                                <td valign="middle"><span class="fw-normal">{{ $sheets02->respondent }}</span></td>
